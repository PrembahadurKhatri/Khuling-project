...
/**
 * Featured MD/CEO message on the homepage — a personal, leadership-voice
 * interlude sitting between "proof of work" (Projects) and "how we work"
 * (Services). Fully data-driven from the admin-managed singleton
 * (services/mdMessageService.js) — pass `mdMessage` from Home.jsx's query
 * and this renders nothing if there's no message set yet, so an empty admin
 * record doesn't leave a bare section on the live site.
 */
const MdCeoMessage = ({ mdMessage }) => {
  if (!mdMessage?.message) return null;
  const { name, designation, message, photo } = mdMessage;
    ...
    >
      ...
      <div className="container-wide py-20 md:py-28 relative">
        <div className="text-center mb-12 md:mb-16">
          ...
          <h2 className="section-title font-body max-w-xl mx-auto">Message from our {designation || "Leadership"}</h2>
        </div>
        ...
        <motion.div
          ...
          className="group relative max-w-4xl mx-auto rounded-3xl border border-line bg-white/80 backdrop-blur-sm
          ...
          <div className="grid md:grid-cols-[280px_1fr]">
            ...
            <div className="p-8 md:p-12 flex flex-col justify-center relative">
              ...
              <div className="mt-6 pt-6 border-t border-line">
                <p className="font-body font-semibold text-navy text-lg">{name}</p>
                {designation && <p className="font-body text-sm text-gold tracking-wide uppercase mt-0.5">{designation}</p>}
              </div>
            </div>
          </div>
        </motion.div>
